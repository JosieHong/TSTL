...
def train_and_evaluate(model_name, task_name, data_dir, checkpoint_path,
	...
	train_dataset, test_dataset, model, collate_fn, train_labels, test_labels = \
		get_dataset_and_model(model_name=model_name, task_name=task_name, root_dir=data_dir, 
	...
	test_metrics = evaluate_model(test_labels, test_predictions)
...
if __name__ == '__main__':
	...
	parser.add_argument('--verbose', action='store_true',
					   help='Print detailed output during training')
	...
	# Start! 
	# =========================================
	if args.n_folds > 1: # k-fold cross-validation
		all_metrics = []
		for fold in range(args.n_folds):
			print(f'- Fold {fold + 1}/{args.n_folds}')

			# Setup checkpoint path for this fold
			checkpoint_path = setup_checkpoint_path(model_name=args.gnn, 
													strategy='sc', 
													stage=None, 
													source_dataset=None, 
		...
		metrics = train_and_evaluate(model_name=args.gnn, task_name=args.target, data_dir=args.data_dir, checkpoint_path=checkpoint_path,
										...
										fold_id=None, n_splits=5, device=args.device, seed=args.seed, verbose=args.verbose)
