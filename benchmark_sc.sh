...
run_experiment() {
    ...
    python -u run_sc.py \
        ...
        --opt adam \
        --device "$device" \
        --n_folds 5 > "./logs/${model}_sc_${TARGET_DATASET}_${timestamp}.log" 2>&1 &
    
    # Store the process ID
    echo $! > "${model}_${TARGET_DATASET}_sc.pid"
}
...
wait_for_batch() {
    ...
    wait
    rm -f *${TARGET_DATASET}_sc.pid
}

# Main execution
echo "Running single-task classification experiments for dataset: $TARGET_DATASET"
